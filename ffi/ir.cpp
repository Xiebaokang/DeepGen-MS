...
using ret = py::return_value_policy;
  ...
  // DeepGen自己的dialect相关的函数，与Python绑定部分
void init_ffi_ir_DeepGen(py::module_ &&m) {
  m.def("load_dialects", [](MLIRContext &context) {
    mlir::DialectRegistry registry;
    // clang-format off
    registry.insert<mlir::DeepGen::DeepGenDialect,
  ...
  py::class_<ForOp, OpState>(m, "ForOp", py::module_local())
      ...
      .def("get_num_stage", &ForOp::getNumStage);
  // alloc buffer op
  py::class_<AllocBufferOp, OpState>(m, "AllocBufferOp", py::module_local())
    .def("get_alignment", &AllocBufferOp::getAlignment)
    .def("get_memroy_space", [](AllocBufferOp &self) -> uint64_t { return self.getMemorySpace(); })
    .def("get_result", [](AllocBufferOp &self) -> Value { return self.getResult(); })
...
void init_ffi_ir_builder(py::module_ &m) {
  ...
  py::class_<OpBuilderWithLoc>(m, "builder")
      ...
      .def("get_e5m2_ty", [](OpBuilderWithLoc &self) -> Type { 
          MLIRContext *ctx = self.getBuilder().getContext();
          return Float8E5M2Type::get(ctx);
        })
      .def("get_e4m3fn_ty", [](OpBuilderWithLoc &self) -> Type { 
          MLIRContext *ctx = self.getBuilder().getContext();
          return Float8E4M3FNUZType::get(ctx);
        })
      .def("get_context", [](OpBuilderWithLoc &self) { 
          return self.getBuilder().getContext();
        })
      ...
      .def("get_memref_ty", 
           [](OpBuilderWithLoc &self, Type &elementType, std::vector<int64_t> &shape, int64_t space, AffineMap map) -> Type {
             if (!map) {
           ...
           }, py::arg("elementType"), py::arg("shape"), py::arg("space") = 1, py::arg("map") = AffineMap())
      .def("get_function_ty",
      ...
      .def("set_loc", [](OpBuilderWithLoc &self, const std::string &fileName, int line, int column) { 
              self.setLastLoc(fileName, line, column); 
            })
      ...
      // DeepGen
      .def("create_gemm_op", 
        [](OpBuilderWithLoc &self, Value &A, Value &B, Value &C, bool transA, bool transB,
           AffineMap AMap, AffineMap BMap, 
           std::vector<Value> AIndices, std::vector<Value> BIndices) {
            if (!AMap && !BMap) {
              return self.create<GemmOp>(A, B, C, transA, transB);
            } else {
              return self.create<GemmOp>(A, B, C, AMap, BMap, AIndices, BIndices, transA, transB);
            }
        },
        py::arg("A"), py::arg("B"), py::arg("C"),
        py::arg("transA") = false, py::arg("transB") = false,
        py::arg("srcMap") = AffineMap(), py::arg("dstMap") = AffineMap(),
        py::arg("srcIndices") = std::vector<Value>(), py::arg("dstIndices") = std::vector<Value>())
        ...
        [](OpBuilderWithLoc &self, std::vector<int64_t> &blockRange, py::function pyFunc) {
          return self.create<BlockOp>(blockRange, [&](ValueRange args) -> void {
            ...
            try {
              ...
              llvm::errs() << "Python exception in DeepGen block op: " << e.what() << "\n";
              throw;
            }
          });
        })
      ...
      .def("create_for_op", 
        [](OpBuilderWithLoc &self, int64_t lower, int64_t upper, int64_t step, py::function pyFunc, int64_t numStage) {
          return self.create<ForOp>(lower, upper, step, [&](Value arg) -> void {
            py::gil_scoped_acquire acquire;
            try {
              pyFunc(self, arg);
            } catch (const py::error_already_set &e) {
              llvm::errs() << "Python exception in DeepGen for op: " << e.what() << "\n";
              throw;
            }
          }, numStage);
        }, py::arg("lower"), py::arg("upper"), py::arg("step"), py::arg("pyFunc"), py::arg("numStage")=1)

      .def("create_alloc_buffer_op", 
        [](OpBuilderWithLoc &self, std::vector<int64_t> shape, const std::string &dtype, int64_t space, int64_t alignment) {
          MLIRContext *context = self.getBuilder().getContext();
          Type ety;
          if (dtype == "e4m3fn") {
            ety = Float8E4M3FNType::get(context);
          } else if (dtype == "e5m2") {
            ety = Float8E5M2Type::get(context);
          } else if (dtype == "float16") {
            ety = Float16Type::get(context);
          } else if (dtype == "float32") {
            ety = Float32Type::get(context);
          } else if (dtype == "float64") {
            ety = Float64Type::get(context);
          } else {
            throw std::invalid_argument("invalid float type");
          }
          if (alignment < 0)
            throw std::invalid_argument("alignment must be non-negative");
          auto resultType = MemRefType::get(shape, ety, AffineMap(), static_cast<unsigned>(space));
          return self.create<AllocBufferOp>(resultType, llvm::ArrayRef<int64_t>(shape), ety, static_cast<uint64_t>(alignment), space);
      }, py::arg("shape"), py::arg("dtype"), py::arg("space") = 1, py::arg("alignment") = 0)
  ...
  py::class_<AffineMap>(m, "AffineMap", py::module_local())
    ...
    .def_static("get", 
      [](unsigned dimCount, unsigned symbolCount, std::vector<AffineExpr> results, MLIRContext *context) -> AffineMap {
        return AffineMap::get(dimCount, symbolCount, llvm::ArrayRef<AffineExpr>(results), context); 
      })
    .def_static("get_constant_map", 
      [](int64_t val, MLIRContext *context) -> AffineMap { return AffineMap::getConstantMap(val, context); })
    .def_static("get_constants_map", 
      [](const std::vector<int64_t> &vals, MLIRContext *context) -> AffineMap { 
        llvm::SmallVector<AffineExpr> exprs = getAffineConstantExprs(vals, context);
        return AffineMap::get(0, 0, llvm::ArrayRef<AffineExpr>(exprs), context); 
      })
    ...
    .def_static("get_permutation_map", 
      [](std::vector<unsigned> permutation, MLIRContext *context) -> AffineMap { 
        return AffineMap::getPermutationMap(llvm::ArrayRef<unsigned>(permutation), context); 
      })
    .def_static("get_permutation_map", 
      [](std::vector<int64_t> permutation, MLIRContext *context) -> AffineMap { 
        ...
        return AffineMap::getPermutationMap(llvm::ArrayRef<unsigned>(unsignedPermutation), context); 
      })
...
void init_ffi_ir_common(py::module_ &m) {
  ...
  py::class_<MemRefType, Type>(m, "memref_buffer", py::module_local())
      ...
      .def("get_memory_space", [](MemRefType &self) -> std::string {
        uint64_t space = self.getMemorySpaceAsInt();
        if (space == 0 || space == 5) return "local";
        else if (space == 3) return "shared";
        // else if (space == 1) return "global";
        return "global";
      });
  ...
  py::class_<Region>(m, "region", py::module_local())
      ...
      .def("front", [](Region &self) -> Block * { return &self.front(); }, ret::reference)
...
void init_ffi_ir(py::module_ &&m) {
  ...
  init_ffi_ir_DeepGen(m.def_submodule("DeepGen"));
}
