...
```bash
git clone https://github.com/Xiebaokang/DeepGen-MS
cd DeepGen-MS
...
```bash
export PYTHONPATH=~/install/path/DeepGen-MS/python:$PYTHONPATH
```
...
```bash
cd ~/install/path/DeepGen-MS/test
python bind_test.py
```
This will generate and execute a GEMM kernel using the DeepGen-MS compiler.
