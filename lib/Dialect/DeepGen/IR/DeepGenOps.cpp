...
#include "Dialect/DeepGen/IR/DeepGenOps.cpp.inc"

// move dialect def in this file to make compiler happy
#include "Dialect/DeepGen/IR/DeepGenDialect.cpp.inc"
...
void KernelOp::print(OpAsmPrinter &p) {
  ...
  p << "(";
  for (int i=0; i<funcTy.getInputs().size(); i++) {
    if (i > 0) p << ", ";
    p << getBody(0)->getArgument(i) << " : " << funcTy.getInput(i);
  }
...
void ForOp::build(OpBuilder &builder, OperationState &state, 
                  ...
                  BodyBuilderFn bodyBuilder, int64_t numStage) {
  OpBuilder::InsertionGuard guard(builder);
  ...
  state.addAttribute("numStage", builder.getI64IntegerAttr(numStage));
...
ParseResult ForOp::parse(OpAsmParser &parser, OperationState &result) {
  // 解析格式：DeepGen.for %arg1 = 0 to 1024 step = 32 { ... }
  OpAsmParser::UnresolvedOperand inductionVar;
  IntegerAttr lowerAttr, upperAttr, stepAttr, numStageAttr;
  ...
  if (parser.parseAttribute(stepAttr, builder.getIntegerType(64), "step", result.attributes))  // 解析步长
    return failure();
  if (parser.parseKeyword("num_stage") || parser.parseEqual())  // 解析 "numStage"
    return failure();
  if (parser.parseAttribute(numStageAttr, builder.getIntegerType(64), "numStage", result.attributes))  // 解析步长
    return failure();
...
void GemmOp::build(OpBuilder &builder, OperationState &state, Value A, Value B, Value C, bool transA, bool transB) {
  // 设置操作数
  auto AType = llvm::cast<MemRefType>(A.getType());
  int64_t rank = AType.getRank();
  SmallVector<AffineExpr> exprs;
  for (unsigned i = 0; i < rank; ++i) {
    exprs.push_back(builder.getAffineConstantExpr(0));
  }
  AffineMap map = AffineMap::get(0, 0, exprs, builder.getContext());
  build(builder, state, A, B, C, map, map, ValueRange(), ValueRange(), transA, transB);
}

void GemmOp::build(OpBuilder &builder, OperationState &state, Value A, Value B, Value C, 
                   AffineMap AMap, AffineMap BMap, ValueRange AIndices, ValueRange BIndices) {
  build(builder, state, A, B, C, AMap, BMap, AIndices, BIndices, false, false);
}
...
ParseResult GemmOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  // 解析源操作数和索引
  OpAsmParser::UnresolvedOperand AMemrefInfo;
  AffineMapAttr AMapAttr;
  SmallVector<OpAsmParser::UnresolvedOperand, 4> AMapOperands;
  if (parser.parseOperand(AMemrefInfo) || parser.parseLSquare() ||
      parser.parseAffineMapOfSSAIds(AMapOperands, AMapAttr, "AMap", result.attributes) ||
      parser.parseRSquare() || parser.parseComma())
    return failure();
  // 解析目标操作数和索引
  OpAsmParser::UnresolvedOperand BMemrefInfo;
  AffineMapAttr BMapAttr;
  SmallVector<OpAsmParser::UnresolvedOperand, 4> BMapOperands;
  if (parser.parseOperand(BMemrefInfo) || parser.parseLSquare() ||
      parser.parseAffineMapOfSSAIds(BMapOperands, BMapAttr, "BMap", result.attributes) ||
      parser.parseRSquare())
    return failure();
  // 解析类型信息
  SmallVector<Type, 3> ABCTypes;
  if (parser.parseColonTypeList(ABCTypes))
    return failure();
  if (ABCTypes.size() != 3)
    return parser.emitError(parser.getNameLoc(), "expected three memref types");
  auto AType = dyn_cast<MemRefType>(ABCTypes[0]);
  auto BType = dyn_cast<MemRefType>(ABCTypes[1]);
  auto CType = dyn_cast<MemRefType>(ABCTypes[2]);
  ...
  if (parser.parseOptionalAttrDict(result.attributes))
    return failure();
  // 解析操作数
  if (parser.resolveOperand(AMemrefInfo, AType, result.operands) ||
      parser.resolveOperand(BMemrefInfo, BType, result.operands) ||
      parser.resolveOperands(AMapOperands, indexTy, result.operands) ||
      parser.resolveOperands(BMapOperands, indexTy, result.operands))
    return failure();
  // 设置 operandSegmentSizes 属性（隐藏的）
  SmallVector<int32_t> segmentSizes = {
      1, 1, 
      static_cast<int32_t>(AMapOperands.size()),  // srcIndices
      static_cast<int32_t>(BMapOperands.size())   // dstIndices
  };
...
void GemmOp::print(OpAsmPrinter &p) {
  p << " (" << getA() << "[";
  // 打印源映射和操作数
  if (AffineMapAttr AMapAttr = (*this)->getAttrOfType<AffineMapAttr>("AMap")) {
    p.printAffineMapOfSSAIds(AMapAttr, getAIndices());
  }
  p << "], " << getB() << "[";
  // 打印目标映射和操作数
  if (AffineMapAttr BMapAttr = (*this)->getAttrOfType<AffineMapAttr>("AMap")) {
    p.printAffineMapOfSSAIds(BMapAttr, getBIndices());
  }
  p << "], " << getC() << ")";
  // 打印属性字典，但要排除已打印的映射属性和 operandSegmentSizes
  SmallVector<StringRef> elidedAttrs = { "AMap", "BMap", 
      CopyOp::getOperandSegmentSizesAttrName((*this)->getName()).getValue()
  };
  p.printOptionalAttrDict((*this)->getAttrs(), elidedAttrs);
  p << " : " << getA().getType() << ", " << getB().getType() << ", " << getC().getType();
}
